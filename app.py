...
from wtforms import StringField, IntegerField, SubmitField, FloatField, TextAreaField
...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:adam@localhost/adambensHW5"
## Provided:
...
def update(item):
    ...
    if form.validate_on_submit():
        if request.method == 'POST':
            ...
            db.session.commit()
            flash("Updated priority of the item: {}".format(it.description))
            return redirect(url_for('all_lists'))
...
def delete(lst):
    ...
    todo = TodoList.query.filter_by(title=lst).first()
    db.session.delete(todo)
    db.session.commit()
    flash('Deleted List: {}'.format(todo.title))
    return redirect(url_for('all_lists'))
